...
class RGBXYZTransfer:
    def __init__(self, color_space, cuda_available=False) -> None:
        ...
        if color_space == 'bt709':
            self.RGB_to_XYZ_matrix = torch.tensor([[0.412391, 0.357584, 0.180481],
                                                  [0.212639, 0.715169, 0.072192],
                                                  [0.019331, 0.119195, 0.950532]], dtype=torch.float32)

            self.XYZ_to_RGB_matrix = torch.tensor([[3.2409663, -1.5373788, -0.49861172],
                                                  [-0.96924204, 1.8759652, 0.04155577],
                                                  [0.05562956, -0.20397693, 1.0569717]], dtype=torch.float32)
        if color_space == 'bt2020':
            self.RGB_to_XYZ_matrix = torch.tensor([[0.636958, 0.144617, 0.168881],
                                                    [0.262700, 0.677998, 0.059302],
                                                    [0.000000, 0.028073, 1.060985]], dtype=torch.float32)

            self.XYZ_to_RGB_matrix = torch.tensor([[1.716651, -0.355671, -0.253366],
...
if __name__ == '__main__':
    ...
    # # verify rgb2xyz
    # randrgb = torch.rand((1080,1920,3), dtype=torch.float32)
